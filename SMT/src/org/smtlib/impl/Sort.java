...
public abstract class Sort extends Pos.Posable implements ISort {
	...
	/** Returns true iff the receiver is the pre-defined Bool sort */
	@Override
	public boolean isBool() {
		return (this instanceof IExpression) && ((IExpression)this).family().toString().equals(BOOL);
		//return this.equals(Bool);
	}

	/** Returns the pre-defined Bool sort */
	static public ISort Bool() {
		return Bool;
	}
	...
	static final private Sort.Expression Bool = new Sort.Expression(new Symbol(BOOL), new LinkedList<ISort>());
	...
	static public class Family implements IFamily {
		...
		@Override
		public IExpression eval(List<ISort> sorts) {
			if (sorts.size() != arity().intValue()) {
				throw new RuntimeException("Incorrect number of arguments"); // FIXME - better error handling
			}
			Expression e = new Expression(this.identifier(),sorts);
			e.definition(this);
			return e;
		}
	...
	static public class Abbreviation implements IAbbreviation {
		...
		public ISort eval(List<ISort> sorts) {
			if (sorts.size() != parameters().size()) {
				throw new SMT.InternalException("Incorrect number of arguments: " + sorts.size() + " instead of " + parameters().size()); // FIXME - better error handling
			}
			Map<IIdentifier,ISort> map = new HashMap<IIdentifier,ISort>();
			int i = 0;
			for (IParameter p: parameters) {
				if (map.put(p.symbol(),sorts.get(i))!=null) {
					throw new SMT.InternalException("Duplicate parameter: " + p); // FIXME - better error handling
				}
			}
	 ...
	 */
	static public class Expression extends Sort implements IExpression {
		protected IIdentifier sortFamily;
		...
		public Expression(IIdentifier sortFamily, List<ISort> sortParameters) {
			this.sortFamily = sortFamily;
			this.sortParameters = sortParameters;
		}
		public Expression(IIdentifier sortFamily, ISort... sortParameters) {
			this.sortFamily = sortFamily;
			this.sortParameters = Arrays.asList(sortParameters);
		}
		@Override
		public IIdentifier family() { return sortFamily; }
		...
		public boolean equals(Object sort) {
			...
			if (!(sort instanceof Expression)) return false;
			// FIXME - fix casts
			ISort.IExpression ethis = (ISort.IExpression)this.eval();
			ISort.IExpression e = (ISort.IExpression)((Expression)sort).eval();
			if (!(e.family().equals(ethis.family()))) return false;
			// If the family() is equal, the arity must be equal
			int i = 0;
			for (ISort p: ethis.parameters()) {
		...
		public ISort eval() {
			return definition().eval(sortParameters);
		}

		@Override
		public boolean equals(Map<IIdentifier,ISort> leftmap, ISort sort, Map<IIdentifier,ISort> rightmap, SymbolTable symTable) {
			if (this == sort) return true;
			if (!(sort instanceof IExpression)) return false;
			IExpression e = (IExpression)sort;
		...
		@Override
		public ISort substitute(Map<IIdentifier,ISort> map) {
			...
			ISort s = map.get(id);
			if (s != null) return s;
			Expression e = new Expression(id,params);
		...
		@Override
		public String toString() {
			if (sortParameters.size() == 0) return sortFamily.toString();
			StringBuilder sb = new StringBuilder();
			sb.append("(");
			sb.append(sortFamily.toString());
			for (ISort s: sortParameters) {
				sb.append(" ");
	...
	static public class FcnSort extends Sort implements IFcnSort {
		...
		@Override
		public ISort substitute(Map<IIdentifier, ISort> map) {
			ISort newResult = ((Sort)resultSort).substitute(map);
	...
	/** Represents a Sort parameter, such as in the definition of a Sort abbreviation */
	static public class Parameter extends Sort implements IParameter {
		...
		public boolean equals(Object o) {
			...
			return ((IParameter)o).symbol().equals(symbol);
		}
