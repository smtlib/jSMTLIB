...
/**
 * This class implements launching, writing to, and reading responses from a launched process (in
 * particular, solver processes).
 *
 * @author David Cok
 */
public class SolverProcess {
  ...
  public static boolean useNotifyWait = false; // TOOD: Not sure that true works
   ...
   * @param logfile if not null, the name of a file to log communications to, for diagnostic
   *     purposes
   */
  static java.io.PrintWriter pw = null;
  /**
   * A pool of buffers used by listenThru. The listenThru method needs a buffer, which may need to
   * be big. However, the method is called often and we do not want to be continually allocating big
   * buffers that have to wait around to be garbage collected. Especially since, unless there are
   * multiple SMT processes working simultaneously, we will never need more than one of these. But
   * in order to be thread-safe we cannot simply declare a static buffer.
   */
  private static List<char[]> bufferCollection =
      Collections.synchronizedList(new LinkedList<char[]>());
  /**
   * A place (e.g., log file), if non-null, to write all outbound communications for diagnostic
   * purposes
   */
  ...
  /**
   * The Reader process that reads from the standard output of the spawned process (initialized by
   * start() )
   */
  protected Reader fromProcess;
  /**
   * The Reader process that reads from the standard error stream of the spawned process
  ...
  public SolverProcess(String[] cmd, String endMarker, /*@Nullable*/ String logfile) {
    ...
    try {
      ...
      System.out.println(
          "Failed to create solver log file "
              + logfile
              + ": "
              + e); // FIXME - wwrite to somewhere better
    }
    setCmd(cmd);
  }
   ...
   */
  private static synchronized void putBuffer(char[] buf) {
    bufferCollection.add(buf);
  }

  /**
   * Reads the given Reader until the given String is read (or end of input is reached); may block
   * until input is available; the stopping string must occur at the end of the input. This is
   * typically used to read through a prompt on standard output; when the stopping string (the
   * prompt) is read, one knows that the output from the program is complete and not to wait for any
   * more.
   *
   ...
   */
  public static /*@NonNull*/ String listenThru(
      /*@NonNull*/ Reader r, /*@Nullable*/ String end) throws IOException {
  ...
  public static void main(String... args) {
    ...
    while (true) {
      ...
      // ((BufferedReader)sp.fromProcess).readLine());
  ...
  public void setCmd(String[] cmd) {
    ...
    try {
      if (log != null && cmd != null) {
        // TODO: Might be nicer to escape any backslashes and enclose strings in quotes, in case
        // arguments contain spaces or special characters
        log.write(";; ");
        for (String s : cmd) {
          log.write(s);
          log.write(" ");
      ...
      System.out.println(
          "Failed to write to solver log file : " + e); // FIXME - wwrite to somewhere better
    }
  }

  /**
   * Starts the process; if the argument is true, then also listens to its output until a prompt is
   * read.
   */
  ...
  /**
   * Listens to the process's standard output until the designated endMarker is read and to the
   * error output. If there is error output, it is returned; otherwise the standard output is
   * returned.
   */
  public String listen() throws IOException {
    // FIXME - need to put the two reads in parallel, otherwise one might block on a full buffer,
    // preventing the other from completing
    if (!useMultiThreading) {
      ...
      if (log != null) {
        if (!out.isEmpty()) {
          ...
          log.write(eol);
          log.flush();
        } // input usually ends with a prompt and no line terminator
        if (!err.isEmpty()) {
          log.write(";ERR: ");
          log.write(err);
        } // input usually ends with a line terminator, we think
      }
      //	      System.out.println("OUT: " + out.replace('\r', '@').replace('\n', '@'));
      //	      System.out.println("ERR: " + err.replace('\r', '@').replace('\n', '@'));
      // In some cases (yices2) the prompt is on the error stream. Our heuristic is that then there
      // is no line-termination
      if (err.endsWith("\n") || out.isEmpty()) {
        return err.isEmpty() || err.charAt(0) == ';'
            ? out
            : err; // Note: the guard against comments (starting with ;) is for Z3
      } else {
        return out;
      }
    } else {
      // FIXME - should not have to busy wait with sleep command and the watsch on a field in
      // another thread
      String out;
      if (useNotifyWait) {
        ...
        try {
          ...
          } while (standardOut.output == null && process.isAlive());
      ...
      String err =
          errorOut
              .getString(); // errorOut is set up to not block -- presuming error output is ready if
      // standard out has completed, since there is no indication when the
      // error output is completed
      if (err.endsWith("\n") || out.isEmpty()) {
        return err.isEmpty() || err.charAt(0) == ';'
            ? out
            : err; // Note: the guard against comments (starting with ;) is for Z3
  ...
  /**
   * Returns true if the process is still running; this relies on exceptions for control flow and
  ...
  public boolean isRunning(boolean expectStopped) {
    ...
    try {
      ...
      if (!expectStopped) {
        if (log != null) {
          try {
            log.write("Solver has unexpectedly terminated");
            log.write(eol);
            log.flush();
          } catch (IOException e) {
            // ignore
  ...
  /**
   * Sends all the given text arguments, then (if listen is true) listens for the designated end
   * marker text
   */
