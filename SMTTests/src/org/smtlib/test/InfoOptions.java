...
public class InfoOptions extends LogicTests {
  ...
  public void checkAuthors() {
    checkGetInfo(
        ":authors",
        (solvername.equals("test")
            ? "David R. Cok"
            : solvername.equals("simplify")
                ? "David Detlefs and Greg Nelson and James B. Saxe"
                : solvername.startsWith("yices")
                    ? "Bruno Dutertre"
                    : solvername.equals("cvc")
                        ? "Clark Barrett, Cesare Tinelli, and others"
                        : solvername.equals("cvc4")
                            ? null // Long text that we don't check // TODO
                            : solvername.equals("cvc4b")
                                ? null // Long text that we don't check // TODO
                                : solvername.startsWith("z3_4_3")
                                    ? "Leonardo de Moura and Nikolaj Bjorner"
                                    : solvername.startsWith("z3_")
                                        ? "Leonardo de Moura, Nikolaj Bjorner and Christoph Wintersteiger"
                    ...
                    : solvername.equals("yices2")
                        ...
                        : solvername.equals("cvc")
                            ...
                            : solvername.equals("cvc4")
                                ...
                                : solvername.equals("cvc4b")
                                    ? "1.5-prerelease"
                                    : solvername.equals("z3_4_3")
                                        ? "4.3"
                                        : solvername.equals("z3_4_3_2")
                                            ? "4.3.2"
                                            : solvername.equals("z3_4_4")
                                                ? "4.4.0"
                                                : solvername.equals("z3_4_5")
                                                    ? "4.5.0"
                                                    : solvername.equals("z3_4_6")
                                                        ? "4.6.0"
                                                        : solvername.equals("z3_4_7")
                                                            ? "4.7.1"
                                                            : solvername.equals("z3_4_8_5")
                                                                ? "4.8.5"
                                                                : solvername.equals("z3_2_11")
                                                                    ? "2.11"
                                                                    : "???"));
  }
  ...
  public void checkRegularOutput() {
    ...
    doCommand(
        ...
        solvername.startsWith("cvc4") ? "unsupported" : "\"stdout\"");
  }
  ...
  public void checkSetRegularOutput() {
    ...
    doCommand(
        "(set-option :regular-output-channel \"test-output\")",
        "success"); // FIXME - writes success to test-output? - hangs for z3_4_3 ?
  ...
  public void checkSetDiagnosticOutput() {
    Assume.assumeTrue(false);
    doCommand(
        "(set-option :regular-diagnostic-channel \"test-output\")",
        "success"); // FIXME - writes success to test-output? - hangs for z3_4_3 ?
  ...
  public void checkSetInteractiveMode() {
    doCommand(
        ...
        solvername.equals("yices2") ? "unsupported" : "success");
    doCommand(
        "(get-option :interactive-mode)", solvername.equals("yices2") ? "unsupported" : "true");
    doCommand(
        "(set-option :interactive-mode false)",
        solvername.equals("yices2") ? "unsupported" : "success");
    doCommand(
        "(get-option :interactive-mode)", solvername.equals("yices2") ? "unsupported" : "false");
  }

  @Test
  public void checkProduceProofs() {
    boolean supported = !solvername.equals("yices2");
    doCommand("(get-option :produce-proofs)", !supported ? "unsupported" : "false");
  }
  ...
  public void checkSetProduceProofs() {
    ...
    doCommand(
        ...
        supported
            ? "success"
            : solvername.startsWith("cvc4")
                ? "(error \"Error in option parsing: option `produce-proofs' requires a proofs-enabled build of CVC4; this binary was not built with proof support\")"
                : "unsupported");
    doCommand(
        "(get-option :produce-proofs)",
        supported ? "true" : solvername.equals("yices2") ? "unsupported" : "false");
    doCommand(
        "(set-option :produce-proofs false)",
        supported ? "success" : solvername.startsWith("cvc4") ? "success" : "unsupported");
    doCommand(
        "(get-option :produce-proofs)", solvername.equals("yices2") ? "unsupported" : "false");
  }
  ...
  public void checkSetProduceModels() {
    boolean support =
        isTest
            || solvername.startsWith("z3")
            || "cvc".equals(solvername)
            || "cvc4".equals(solvername)
            || "cvc4b".equals(solvername)
            || "yices2".equals(solvername);
  ...
  public void checkSetProduceAssignments() {
    boolean supported =
        isTest
            || solvername.startsWith("cvc4")
            || solvername.equals("yices2")
            || (!solvername.equals("z3_4_3") && solvername.startsWith("z3_"));

    doCommand("(set-option :produce-assignments true)", supported ? "success" : "unsupported");
    doCommand("(get-option :produce-assignments)", supported ? "true" : "false");
    doCommand("(set-option :produce-assignments false)", supported ? "success" : "unsupported");
    doCommand("(get-option :produce-assignments)", "false");
  }
  ...
  public void checkProduceUnsatCores() {
    ...
    doCommand("(get-option :produce-unsat-cores)", unsupported ? "unsupported" : "false");
  }
  ...
  public void checkSetProduceUnsatCores() {
    ...
    doCommand(
        ...
        solvername.equals("yices2") ? "unsupported" : supported ? "true" : "false");
    doCommand(
        "(set-option :produce-unsat-cores false)",
        supported ? "success" : solvername.startsWith("cvc4") ? "success" : "unsupported");
    doCommand(
        "(get-option :produce-unsat-cores)", solvername.equals("yices2") ? "unsupported" : "false");
  }
  ...
  public void checkExpandDefinitions() {
    ...
    doCommand("(get-option :expand-definitions)", supported ? "false" : "unsupported");
  }

  @Test
  public void checkSetExpandDefinitions() {
    boolean supported =
        smt.smtConfig.isVersion(SMT.Configuration.SMTLIB.V20)
            && !solvername.equals("yices2")
            && !solvername.equals("z3_4_4")
            && !solvername.equals("z3_4_5");
    supported = true; // FIXME - it is optional, but it is permitted to set
    doCommand("(set-option :expand-definitions true)", !supported ? "unsupported" : "success");
    doCommand("(get-option :expand-definitions)", !supported ? "unsupported" : "true");
    doCommand("(set-option :expand-definitions false)", !supported ? "unsupported" : "success");
    doCommand("(get-option :expand-definitions)", !supported ? "unsupported" : "false");
  }
  ...
  public void checkSetExpandDefinitions2() {
    ...
    doCommand(
        "(get-option :expand-definitions)", (solvername.startsWith("z3")) ? "true" : "unsupported");
  ...
  public void checkRandomSeed() {
    Assume.assumeTrue(
        !"cvc4".equals(solvername)); // FIXME - cvc4 does not handle randome seed correctly
  ...
  public void checkSetRandomSeed() {
    ...
    doCommand("(get-option :random-seed)", "cvc4".equals(solvername) ? "0" : "1");
    doCommand("(set-option :random-seed 2)", "success");
    doCommand("(get-option :random-seed)", "cvc4".equals(solvername) ? "0" : "2");
  }
