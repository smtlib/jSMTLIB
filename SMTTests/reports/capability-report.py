...
Usage:
  python3 capability-report.py [--solver-dir DIR] [--solvers NAME[,NAME...]]
...
runs every probe, and writes ALL-report.md.
"""
...
def render_table(header: list[str], rows: list[list[str]]) -> list[str]:
    ...
    width, so it's readable as plain text (not just when rendered), e.g. in
    a text editor or a terminal `cat`. Column width is character-count based
    (not display-width aware), so rows mixing the warning emoji -- which is
    two Unicode code points -- with the other single-code-point grade emoji
    can be off by a column or two in a real editor; exact emoji-width
    alignment isn't worth a wcwidth dependency here."""
...
def render_markdown(
    ...
    lines.append("")
    lines.append(
        "Generated by `SMTTests/capability-report.py`. Every SMT-LIB-compliant "
...
def main() -> int:
    ...
    ap.add_argument("--out", type=Path, default=Path("ALL-report.md"), help="Output file (default: ALL-report.md; pass - for stdout)")
    args = ap.parse_args()
