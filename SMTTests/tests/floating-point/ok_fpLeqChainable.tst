; fp.leq is :chainable. 1.0 <= 2.0 <= 3.0 is true; 1.0 <= 3.0 <= 2.0 is false
; (3.0 <= 2.0 fails) -- all exact in binary32, no rounding subtlety.
(set-logic ALL)
...
(assert (fp.leq one two three))
(check-sat)
(assert (fp.leq one three two))
(check-sat)
